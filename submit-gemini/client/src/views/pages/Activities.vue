        ...
        <el-table v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
          ...
          <el-table-column align="center" label="参与人员">
            <template slot-scope="scope">
              <el-popover title="活动参与人员" trigger="click" @show="getActivePeople(scope.row.id)" :width="600" placement="left">
                <el-table :data="activeLogs" border>
                  <el-table-column align="center" type="index"></el-table-column>
                  <el-table-column align="center" label="头像" width="80">
                    <template slot-scope="pScope">
                      <el-avatar :size="30" :src="getFileUrl(pScope.row.userAvatar)" icon="el-icon-user-solid"></el-avatar>
                    </template>
                  </el-table-column>
          ...
          <el-table-column v-if="userType == 0" align="center" label="操作处理">
            <template slot-scope="scope">
              <el-button icon="el-icon-delete" type="danger" size="mini" @click="delInfo(scope.row.id)"></el-button>
            </template>
          </el-table-column>
            ...
            <template slot-scope="scope">
              <el-popover trigger="click" :width="700" placement="left">
                ...
                <el-descriptions :column="1" size="small" border>
                  ...
                  <el-descriptions-item>
                    <template slot="label">活动风采</template>
                    <div v-if="scope.row.mediaFile">
                      <img v-if="isImage(scope.row.mediaFile)" :src="getFileUrl(scope.row.mediaFile)" style="max-width: 100%; max-height: 300px; border-radius: 4px;">
                      <video v-else-if="isVideo(scope.row.mediaFile)" :src="getFileUrl(scope.row.mediaFile)" controls style="max-width: 100%; max-height: 300px;"></video>
                      <a v-else :href="getFileUrl(scope.row.mediaFile)" target="_blank" style="color: #009999;">点击下载查看附件</a>
                    </div>
                    <span v-else style="color: #999;">暂无展示素材</span>
                  </el-descriptions-item>
    ...
    <el-dialog
      ...
      <el-form label-width="90px" :model="activitiesForm">
        ...
        <el-form-item label="活动概述">
          <el-input type="textarea" rows="6" v-model="activitiesForm.comm" placeholder="请输入活动概述…" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="活动要求">
          <el-input type="textarea" rows="6" v-model="activitiesForm.ask" placeholder="请输入活动要求…" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="活动详情">
          <el-input type="textarea" rows="10" v-model="activitiesForm.detail" placeholder="请输入活动详情…" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
